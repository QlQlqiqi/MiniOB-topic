...
RC IvfflatIndex::build_index()
{
  ...
  bool ret;
  switch (static_cast<FunctionExpr::Type>(func_type_)) {
    case FunctionExpr::Type::L2_DISTANCE: ret = index_l2_disance_->build(std::min(lists_, 60)); break;
    case FunctionExpr::Type::COSINE_DISTANCE: ret = index_cosine_->build(std::min(lists_, 60)); break;
    case FunctionExpr::Type::INNER_PRODUCT: ret = index_inner_product_->build(std::min(lists_, 60)); break;
  }
  return ret ? RC::SUCCESS : RC::INTERNAL;
}
