...
RC BplusTreeHandler::create(LogHandler &log_handler, BufferPoolManager &bpm, const char *file_name,
    const std::vector<int> &field_ids, const std::vector<const FieldMeta *> &fields, const bool unique, /* = false */
    int internal_max_size /* = -1*/, int leaf_max_size /* = -1 */)
{
  ...
  rc = this->create(log_handler, *bp, field_ids,fields,unique, internal_max_size, leaf_max_size);
  if (OB_FAIL(rc)) {
...
RC BplusTreeHandler::create(LogHandler &log_handler, DiskBufferPool &buffer_pool, const std::vector<int> &field_ids,
    const std::vector<const FieldMeta *> &fields, const bool unique, /* = false */
    int internal_max_size /* = -1 */, int leaf_max_size /* = -1 */)
{
  int attr_length = 0;
...
MemPoolItem::item_unique_ptr BplusTreeHandler::make_key(const char *user_key, const RID &rid)
{
  ...
  for (int i = 0; i < file_header_.attr_num; i++) {
    memcpy(static_cast<char *>(key.get()) + offset, user_key + offset, file_header_.attr_length[i]);
...
MemPoolItem::item_unique_ptr BplusTreeHandler::make_key_from_full_record(const char *record, const RID &rid)
{
  MemPoolItem::item_unique_ptr key = mem_pool_item_->alloc_unique_ptr();
  if (key == nullptr) {
    LOG_WARN("Failed to alloc memory for key.");
...
RC BplusTreeHandler::insert_entry(const char *user_key, const RID *rid)
{
  ...
  MemPoolItem::item_unique_ptr pkey = make_key_from_full_record(user_key, *rid);
  if (pkey == nullptr) {
    LOG_WARN("Failed to alloc memory for key.");
    return RC::NOMEM;
  }
...
RC BplusTreeScanner::open(const char *left_user_key, int left_len, bool left_inclusive, const char *right_user_key,
    int right_len, bool right_inclusive, const std::shared_ptr<FieldMeta> field_meta /* =nullptr */)
{
  RC rc = RC::SUCCESS;
  if (inited_) {
  ...
  if (left_user_key && right_user_key) {
    ...
    const int   result          = attr_comparator(left_user_key, right_user_key, field_meta);
